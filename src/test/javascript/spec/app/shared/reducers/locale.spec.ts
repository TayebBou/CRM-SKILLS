...
describe('Locale reducer tests', () => {
  ...
  it('should correctly set the first time locale', () => {
    const localeState = locale(undefined, { type: ACTION_TYPES.SET_LOCALE, locale: 'fr' });
    expect(localeState).toMatchObject({
      currentLocale: 'fr',
    });
    expect(TranslatorContext.context.locale).toEqual('fr');
  });

  it('should correctly detect update in current locale state', () => {
    TranslatorContext.setLocale('fr');
    expect(TranslatorContext.context.locale).toEqual('fr');
    const localeState = locale({ currentLocale: 'fr' }, { type: ACTION_TYPES.SET_LOCALE, locale: 'es' });
    expect(localeState).toMatchObject({
      currentLocale: 'es',
    });
